...
export async function verifyAllInscriptions(address) {
  ...
  const [metaplex, v1, libreplex] = await Promise.all([verifyMetaplex(address), findV1Inscription(address), verifyLibreplex(address)]);
  const valid = metaplex.status === 'valid' || v1.status === 'valid' || libreplex.status === 'valid';
  const unknown = metaplex.status === 'unknown' || v1.status === 'unknown' || libreplex.status === 'unknown';
  const primary = metaplex.status === 'valid' ? metaplex : libreplex.status === 'valid' ? libreplex : v1.status === 'valid' ? v1 : {};
  return {
    ...primary,
