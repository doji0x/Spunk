...
import { verifyBoundToken } from './verifyBoundToken.ts';
...
export async function verifyAllInscriptions(address) {
  ...
  const [metaplex, v1, libreplex, held, bound] = await Promise.all([verifyMetaplex(address), findV1Inscription(address), verifyLibreplex(address), verifyHeldInscription(address), verifyBoundToken(address)]);
  const all = [metaplex, v1, libreplex, held, bound];
  const valid = all.some(check => check.status === 'valid');
  const unknown = all.some(check => check.status === 'unknown');
  const primary = bound.status === 'valid' ? bound : metaplex.status === 'valid' ? metaplex : libreplex.status === 'valid' ? libreplex : v1.status === 'valid' ? v1 : held.status === 'valid' ? held : {};
  return {
    ...primary,
    ...
    checks: { metaplex, v1, libreplex, held, bound }
  };
}