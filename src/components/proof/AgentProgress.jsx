...
export default function AgentProgress({ refreshKey = 0 }) {
  const { data: record, isPending, isError } = useQuery({
    queryKey: ['agentProgress', refreshKey],
    queryFn: async () => {
  ...
  if (!record) return <section className="mt-12 rounded-2xl border border-border bg-card p-5 text-center text-xs text-muted-foreground">{isPending ? 'Loading inscription progress…' : isError ? 'Progress unavailable. Reconnecting automatically…' : 'Waiting for the next agent-endpoint inscription.'}</section>;
  ...
  return <section className="mt-12 rounded-2xl border border-primary/20 bg-card p-5" aria-live="polite">
    ...
    <p className="mt-3 text-xs text-muted-foreground">{complete ? 'Verified on-chain · Complete' : failed ? 'Inscription stopped · Review the admin mint logs' : percent === 100 ? 'Verifying and finalizing…' : 'Queued / writing audio bytes…'}{isError && ' · Connection interrupted; retrying automatically.'}</p>
