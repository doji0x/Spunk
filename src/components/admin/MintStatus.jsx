...
export default function MintStatus({ busy, progress, error, pending, result, onResume, activity, logs }) {
  ...
  return <section aria-live="polite" className="mt-5 rounded-2xl border border-[#dce1d5] bg-white p-5 text-sm">
    {(busy || pending) && <><div className="mb-2 flex justify-between gap-4"><span>{activity || 'Continuing inscription'}</span><span className="shrink-0 font-mono">{progress}%</span></div><Progress value={progress} /><p className="mt-2 text-xs text-muted-foreground">You can safely return and resume if loading is interrupted; confirmed chunks are saved and skipped.</p><PartialInscriptionPreview pending={pending} /></>}
