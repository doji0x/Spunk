...
export default function MintStatus({ busy, progress, error, pending, result, onResume, activity, logs }) {
  ...
  return <section aria-live="polite" className="mt-5 rounded-2xl border border-border bg-card p-5 text-sm">
    ...
    {result && <div className="flex gap-3"><CheckCircle2 className="h-5 w-5 shrink-0 text-primary" /><div><p className="font-semibold">NFT and image fully inscribed and re-verified from chain.</p><a className="mt-1 block break-all font-mono text-xs text-primary underline" href={`https://solscan.io/token/${result.mint}`} target="_blank" rel="noreferrer">{result.mint}</a><p className="mt-2 break-all font-mono text-xs text-muted-foreground">SHA-256: {result.hash}</p><p className="mt-2 text-xs text-muted-foreground">Owned by the server mint wallet: {result.owner}</p>{result.signer && <p className="mt-1 break-all font-mono text-xs text-muted-foreground">Signed by admin wallet: {result.signer}</p>}{result.archivedImageUri && <p className="mt-1 text-xs text-muted-foreground">Source image archived for permanent recovery.</p>}</div></div>}
    <InscriptionLog entries={logs} title="Server inscription log" />
  </section>;
}