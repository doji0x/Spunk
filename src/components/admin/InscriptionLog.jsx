...
export default function InscriptionLog({ entries = [] }) {
  if (!entries.length) return null;
  return <div className="mt-5 border-t pt-4">
    <div className="mb-3 flex items-center gap-2 font-medium"><ScrollText className="h-4 w-4" />Continuity log</div>
    <ol className="max-h-56 space-y-3 overflow-y-auto pr-2">
      {[...entries].reverse().map(entry => <li key={entry.id} className="grid grid-cols-[5rem_1fr] gap-3 text-xs">
