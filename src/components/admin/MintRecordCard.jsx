...
export default function MintRecordCard({ record, onSave, onRetry }) {
  ...
  return <article className="rounded-3xl border border-border bg-card p-5 transition hover:border-primary/25">
    ...
    <dl className="mt-5 grid gap-3 text-xs sm:grid-cols-2">
      ...
      <div><dt className="text-muted-foreground">Archived source image</dt><dd className="mt-1 break-all font-mono">{record.archivedImageUri ? 'Stored' : '—'}</dd></div>
    ...
    <div className="mt-5 flex flex-wrap gap-3">{record.status === 'failed' && record.imageUri ? <Button type="button" size="sm" onClick={() => onRetry(record)}><RotateCcw className="mr-2 h-4 w-4" />Retry background job</Button> : record.status !== 'success' ? <Button asChild size="sm"><Link to={`/admin/mint?mint=${encodeURIComponent(record.mint)}`}>{record.imageUri ? 'View live progress' : 'Move to background'}</Link></Button> : null}<Button asChild size="sm" variant="outline"><a href={`https://solscan.io/token/${record.mint}`} target="_blank" rel="noreferrer">View on Solscan<ExternalLink className="ml-2 h-4 w-4" /></a></Button><Button type="button" size="sm" variant="outline" onClick={() => setEditing(value => !value)}><RefreshCw className="mr-2 h-4 w-4" />Re-upload metadata</Button></div>
