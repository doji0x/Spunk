...
export default function ImageUploadField({ disabled, onFileChange, restoredFile }) {
  ...
  return <div className="space-y-2"><Label htmlFor="image">Inscribed image</Label><Input id="image" type="file" accept="image/png,image/jpeg,image/gif,image/webp" onChange={selectFile} required disabled={disabled} />{error && <p className="text-xs text-destructive">{error}</p>}{preview && <div className="overflow-hidden rounded-lg border bg-muted"><Image src={preview} alt="Selected image preview" fittingType="fit" className="h-48 w-full transition-transform duration-500 hover:scale-[1.02]" /><p className="flex items-center gap-2 border-t bg-card px-3 py-2 text-xs text-muted-foreground"><CheckCircle2 className="h-4 w-4 text-primary" />Validated · {fileInfo.name} · {(fileInfo.size / 1024).toFixed(1)} KB</p></div>}<p className="text-xs text-muted-foreground">PNG, JPEG, GIF, or WebP · 1 MB maximum.</p></div>;
}