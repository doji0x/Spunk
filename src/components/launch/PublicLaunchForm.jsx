...
export default function PublicLaunchForm({ state }) {
  ...
  return <form onSubmit={launch} className="space-y-6 text-left">
    ...
    <Button size="lg" className="gold-glow w-full rounded-full px-8 font-semibold sm:w-auto" disabled={busy || wallet.network !== 'mainnet-beta'}>{busy ? <><Loader2 className="animate-spin" />Waiting for Phantom…</> : <><Rocket />{wallet.address ? 'Review launch in Phantom' : 'Connect Phantom to launch'}</>}</Button>
  </form>;
}