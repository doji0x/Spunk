...
export default function NormalLaunchStatus({ attempt, busy, onCheck, onResume }) {
  ...
  return <section className="space-y-4 rounded-2xl border border-primary/25 bg-card p-5">
    ...
    {confirmed && <Link to={`/edit-coin?coin=${encodeURIComponent(attempt.coinMint)}`} className="inline-flex text-sm text-primary underline underline-offset-4">Update name, image & links</Link>}
