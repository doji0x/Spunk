...
export default function PublicLaunchResult({ result }) {
  ...
  return <section aria-live="polite" className="mt-6 rounded-2xl border border-border bg-card p-5 text-left">
    ...
    {confirmed && <Link to={`/edit-coin?coin=${encodeURIComponent(result.coinMint)}`} className="mt-4 inline-block text-sm text-primary underline underline-offset-4">Update name, image & links</Link>}
