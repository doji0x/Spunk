...
export default function PublicLaunchAdvancedOptions({ input, setInput, settings, disabled }) {
  ...
  return <details open className="rounded-2xl border border-border bg-card/60 p-4">
    <summary className="cursor-pointer font-semibold text-primary">Pair, rewards & social links</summary>
    <div className="mt-5 space-y-6">
      ...
      <p className="text-xs leading-5 text-muted-foreground">Custom fee splits are configured in a separate wallet transaction after the coin creation and required first buy confirm.</p>
    </div>
  </details>;
}