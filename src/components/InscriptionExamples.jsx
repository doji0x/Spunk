...
export default function InscriptionExamples({ onValidate, disabled }) {
  ...
  const discover = async () => {
    ...
    try {
      ...
      setMessage(verified.length ? 'These images passed on-chain verification and image decoding. Select a mint to check it again.' : data.cursor ? 'No displayable image found in this batch. Search the next batch.' : 'No displayable examples found in this search. This does not prove that none exist.');
  ...
  return <section className="mt-4 rounded-xl border border-border bg-card p-4 text-left text-xs">
    ...
    <p className="mt-2 text-muted-foreground">Helius discovery · Metaplex images verified through the private RPC.</p>
    {message && <p aria-live="polite" className="mt-3 text-muted-foreground">{message}</p>}
    <ul className="mt-2 space-y-2">{items.map(item => <li key={item.mint}><button type="button" disabled={loading || disabled} onClick={() => onValidate(item.mint)} className="w-full break-all rounded-md border border-border p-3 text-left font-mono hover:bg-muted disabled:opacity-50">{item.name && <span className="mb-1 block font-body">{item.name}</span>}{item.mint}<span className="mt-1 block font-body text-muted-foreground">Verified image · Validate again</span></button></li>)}</ul>
  </section>;
}