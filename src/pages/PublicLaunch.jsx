...
export default function PublicLaunch() {
  ...
  return <div className="validate-surface min-h-screen text-foreground">
    ...
    <div className="mx-auto grid max-w-5xl gap-10 px-4 py-8 pb-24 sm:px-6 lg:grid-cols-[1fr_360px]">
      <div><div className="mb-7"><h2 className="font-display text-3xl font-bold tracking-tight">Launch from your wallet</h2><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Connect Phantom and launch a pump.fun coin from any valid inscribed NFT you hold or control. You remain the creator and approve the Solana transaction.</p><p className="mt-3 max-w-xl rounded-xl border border-primary/25 bg-primary/5 p-3 text-xs leading-5 text-muted-foreground"><span className="font-semibold text-primary">Anti-bundle protection:</span> coin launch and developer buy are two separate functions and transactions.</p></div><PublicLaunchForm state={state} /><PublicLaunchResult result={state.result} /></div>
