...
import AtomicV1MessageReview from '@/components/atomic/AtomicV1MessageReview';
...
export default function PublicAtomicV1Launch() {
  ...
  return <div className="validate-surface min-h-screen text-foreground">
    ...
    <main className="mx-auto max-w-2xl space-y-7 px-4 py-9 pb-24 sm:px-6">
      ...
      <p className="rounded-xl border border-primary/40 p-4 text-sm"><strong>Experimental Phantom message signing.</strong> Launch will show a transaction-authorization review before requesting a message signature. That signature can authorize coin creation and spending, not merely log you in. Phantom may reject this request.</p>
      <AtomicV1MessageReview review={state.messageReview} onDecision={state.decideMessageReview} />
      ...
      {saved && <section className="space-y-3 rounded-2xl border border-border bg-card p-5">
        ...
        <p className="text-xs text-muted-foreground">First-buy maximum: {session.input?.firstBuyAmount || '0'} SOL. Full transaction size: {state.size?.finalSerializedTransactionBytes ?? 'not prepared'} bytes.</p>
      ...
      <AtomicV1Result walletFailure={failure?.source === 'Phantom'} result={result} onCheck={state.check} busy={busy} linksTo="" />
